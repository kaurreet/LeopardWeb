			...
			if (userType == "student") {
				...
				while (hasExitUser == false) {
					...
					else if (user_input == 2)
					{
						...
						string check = "SELECT COUNT(*) FROM COURSES WHERE TITLE = '" + course_add_drop + "';";
						int course_exists = 0;
						...
						if (course_exists == 0) {
							...
							return_v = studentUser.add_course(db, course_add_drop, studentUser.get_ID());
					...
					else if (user_input == 3)
					{
						...
						//cout << "How many credits is the course worth?: ";
						//cin >> course_credits;
						return_v = studentUser.remove_course(db, course_add_drop, studentUser.get_ID());
						int rc = sqlite3_exec(db, return_v.c_str(), nullptr, nullptr, &errMsg);
						if (rc != SQLITE_OK) {
				...
				while (hasExitUser == false) {
					...
					else if (user_input == 3)
					{
						...
						//cout << "How many credits is the course worth?: ";
						//cin >> course_credits;
						return_v = instructorUser.search_course(db, course_add_drop, instructorUser.get_first_name(), instructorUser.get_last_name());
						char* errMsg = nullptr;
